...
function step(state,id,plan={}){
 ...
 const d={id:s.decisions.length+1,action:id,label:a.label,group:a.group,start:s.time,before:metrics(s),knownEvidence:copy(s.evidence),plan:{hypothesis:plan.hypothesis||'unknown',purpose:plan.purpose||'investigate',prediction:plan.prediction||'',guard:plan.guard||'',note:plan.note||'',evidenceIds:cited},completed:false};
...
function review(s){
 ...
 const details=interventions.map(d=>{
  ...
  const diagnostic=(p.hypothesis!=='unknown'?2:0)+(p.prediction?2:0)+(refs.length?4:0)+(p.guard?2:0);
  ...
  const safety=(p.guard?4:0)+(compatible?3:0)+(checked?3:0);
 ...
 return {scores,details,rootCause:scenario.review.rootCause,stable};
}
